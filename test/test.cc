...
void run_metis_internal(const char* const mat_name,
                        int64_t* const ordering,
    ...
    Eigen::loadMarket(A,
                      fmt::format("/home/aaron/SMITE/test/matrices/{}/{}.mtx",
                                  mat_name,
                                  mat_name));
    ...
    if (n_ordering != perm.rows()) {
        throw std::runtime_error(fmt::format(
                "Ordering size should be {}, got {}", perm.rows(), n_ordering));
    }
...
extern "C" bool run_metis_ffi(const char* const mat_name,
                          int64_t* const ordering,
                          const int64_t n_ordering) {
    try {
        run_metis_internal(mat_name, ordering, n_ordering);
        return true;
